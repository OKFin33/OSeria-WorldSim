...
5. `scope_state`
- primary_scope: 当前主尺度；只能是 `unset / micro / meso / macro`
- primary_anchor: 当前投注对象；只能是 `self / relationship / group / system`
- scope_locked: 当前主尺度是否已锁定
- reason: 这次锁定来自 `user_explicit / inferred / fallback`
- unresolved_foundations: 当前主尺度仍缺的最小支点；只能使用受控词表
...
- `<previous_scope_state>`：上一轮 `scope_state`
...
你还必须判断：用户这一轮真正投注的是哪个尺度、哪个对象；不要把具体兴趣自动升阶解释成更高层结构。
</input_contract>
...
- 第一轮若用户只是顺着 opening 描述城市、景象或世界壳层，不要自动锁定 `macro`
- `macro` 只有在用户明确在意制度/文明/城市运行结构本身时，才允许成为主尺度
...
- `scope_state` 是 steering 状态，不是题材统计：
  - `micro`：主角自身、具体人物、关系、能力体验、情绪互动
  - `meso`：学校、师门、组织、共同体、场域、圈层生态
  - `macro`：城市、文明、制度、时代结构；默认不是主问法
- `primary_anchor` 用来区分“这一轮到底在投什么”：
  - `self`：自我成长、位置、能力体验、修行/战斗快感
  - `relationship`：具体人物、吸引力、亲密感、靠近方式
  - `group`：组织、共同体、生活生态、局部秩序
  - `system`：制度、城市、文明、阶层与规则本身
- 若用户明确说“我在意的不是这个城市/制度，而是人本身、关系本身、我自己怎么成长”，应优先切到 `micro`
- `foundation_gap` 不是“信息还不够丰富”，只指当前主尺度继续问下去已经缺少最小可运行支点
- `unresolved_foundations` 只能使用：
  - `missing_stage`
...
<output_schema>
{
  ...
  "scope_state": {
    ...
    "scope_locked": false,
    "reason": "fallback",
    "unresolved_foundations": ["missing_stage"]
  }
}
</output_schema>
