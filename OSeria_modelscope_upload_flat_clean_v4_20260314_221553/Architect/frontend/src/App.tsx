...
import type { ReplayInspectorTab } from "./components/ReplayLab";
import type {
  ApiErrorPayload,
  Blueprint,
  BubbleCandidate,
...
const REPLAY_SEED_VERSION = "2026-03-14-night-city-v1";
const SHOULD_REGISTER_REPLAY_SEEDS = import.meta.env.DEV && import.meta.env.MODE !== "test";
const ReplayLab = import.meta.env.DEV
  ? lazy(async () => {
      const module = await import("./components/ReplayLab");
      return { default: module.ReplayLab };
    })
  : null;
...
function blueprintToClipboard(blueprint: Blueprint): string {
  return [
    ...
    "",
    "主角冻结身份：",
    `- 姓名：${blueprint.protagonist_name}`,
    `- 性别：${blueprint.protagonist_gender}`,
    `- 身份：${blueprint.protagonist_identity_brief}`,
    "",
    "基调关键词：",
...
async function registerReplaySeeds(existing: ReplayBundle[]): Promise<ReplayBundle[]> {
  if (!SHOULD_REGISTER_REPLAY_SEEDS) {
    return existing;
  }

  const { DEV_REPLAY_SEED_BUNDLES } = await import("./dev/replaySeedBundles");
...
export default function App() {
  ...
  useEffect(() => {
    if (import.meta.env.DEV) {
      ...
      void registerReplaySeeds(stored).then((seeded) => {
        setReplayBundles(seeded);
        if (!stored[0] && seeded[0]) {
          setSelectedReplayBundleId(seeded[0].id);
        }
      });
    }
    void boot();
  ...
  async function handleLandingSubmit(payload: LandingPayload) {
    if (isReplayActive || !sessionId) {
      return;
    }
  ...
  async function handleLaunchRuntime() {
    ...
    try {
      const response = await createRuntimeSession({
        ...
        protagonist_name: blueprint.protagonist_name,
        protagonist_gender: blueprint.protagonist_gender,
        protagonist_identity_brief: blueprint.protagonist_identity_brief,
      ...
      const runtimeAppUrl = import.meta.env.VITE_RUNTIME_APP_URL ?? `${window.location.origin}/runtime/`;
          ...
          <>
            <section className="upper-zone">
              <CurrentScene
                ...
                tutorialHint={uiPhase === "q1" ? tutorialHint : null}
                isWaiting={showTransientWaiting}
                waitingPhrase={waitingPhrase}
              />
            </section>
